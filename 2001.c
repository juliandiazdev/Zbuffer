...
int main() {
  ...
  double dh=0.001;
  int q, fnum=0;
  int earthpic, shippic;
  int earthpicdata[2], shippicdata[2];
  double picdatau, picdatav;
  ...
  earthpic = init_xwd_map_from_file("earth.xwd"); if (earthpic==-1) {printf("Can't load earthpic");}
  get_xwd_map_dimensions(earthpic, earthpicdata);
  shippic = init_xwd_map_from_file("metal.xwd"); if (shippic==-1) {printf("Can't load shippic");}
  get_xwd_map_dimensions(shippic, shippicdata);
  ...
  Mtype[i] = TX; Mparam[i] = 50 ; i++;
  Mtype[i] = TY; Mparam[i] = 10; i++;
  Mtype[i] = TZ; Mparam[i] = 75;

  M3d_make_movement_sequence_matrix(k, ki, 4, Mtype, Mparam);

  i = 0;
  //torus 1 (front) !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

  Mtype[i] = SX; Mparam[i] = 3 ; i++;
  Mtype[i] = SY; Mparam[i] = 3 ; i++;
  Mtype[i] = SZ; Mparam[i] = 3 ; i++;
  Mtype[i] = TX; Mparam[i] = -50 ; i++;
  Mtype[i] = TZ; Mparam[i] = -5;

  M3d_make_movement_sequence_matrix(k1, k1i, 4, Mtype, Mparam);
  ...
  Mtype[i] = SY; Mparam[i] = 3; i++;
  Mtype[i] = SZ; Mparam[i] = 3; i++;
  Mtype[i] = TX; Mparam[i] = -70 ; i++;
  Mtype[i] = TZ; Mparam[i] =  -10;

  M3d_make_movement_sequence_matrix(k2, k2i, 4, Mtype, Mparam);
  ...
  // M3d_make_movement_sequence_matrix(t1bu, t1bui, 4, Mtype, Mparam);
  
  //i = 0;
    
  //torus 1 bridge right !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  // M3d_make_movement_sequence_matrix(t1br, t1bri, 4, Mtype, Mparam);
  ...
  // M3d_make_movement_sequence_matrix(t1bd, t1bdi, 4, Mtype, Mparam);
  ...
  // M3d_make_movement_sequence_matrix(t1bl, t1bli, 4, Mtype, Mparam);
  ...
  // M3d_make_movement_sequence_matrix(t1h, t1hi, 4, Mtype, Mparam);
  ...
  // M3d_make_movement_sequence_matrix(t2bu, t2bui, 4, Mtype, Mparam);
  
  //i = 0;
  
  //torus 2 bridge right !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  // M3d_make_movement_sequence_matrix(t2br, t2bri, 4, Mtype, Mparam);
  ...
  // M3d_make_movement_sequence_matrix(t2bd, t2bdi, 4, Mtype, Mparam);
  ...
  // M3d_make_movement_sequence_matrix(t2bl, t2bli, 4, Mtype, Mparam);
  ...
  // M3d_make_movement_sequence_matrix(t2h, t2hi, 4, Mtype, Mparam);
  ...
  while (q!='q') {
    ...
    eye[0] = -30*cos(2*M_PI*t);
    eye[1] = 0;
    eye[2] = -30*sin(2*M_PI*t);
    ...
    ////earth !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    G_rgb(0,1,0);
    irgb[0]=0, irgb[1]=1; irgb[2]=0;
    for (double u=-M_PI; u<M_PI; u+=del) {
      for (double v=-M_PI/2; u<M_PI/2; v+=del) {
	r=cos(v);
        m[0] = r*cos(u);
        m[1] = sin(v);
        m[2] = r*sin(u);
	...
	M3d_mat_mult_pt(m, k, m);
	M3d_mat_mult_pt(m, view, m);
	
	M3d_mat_mult_pt(m1, k, m1);
	M3d_mat_mult_pt(m1, view, m1);
	
	M3d_mat_mult_pt(m2, k, m2);
    ...
    //torus 1 (front) !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    G_rgb(1,0,0);
    irgb[0]=1; irgb[1]=0; irgb[2]=0;
    for (double u=-M_PI; u<M_PI; u+=del) {
      for(double v=-M_PI; v<M_PI; v+=del){
	...
	i=0;
	Mtype[i] = RX; Mparam[i] = 1.25*fnum; i++;
	M3d_make_movement_seuence_matrix(k1, k1i, 1, Mtype, Mparam);

	M3d_mat_mult_pt(m, k1, m);
	M3d_mat_mult_pt(m, view, m);
	
	M3d_mat_mult_pt(m1, k1, m1);
	M3d_mat_mult_pt(m1, view, m1);
	
	M3d_mat_mult_pt(m2, k1, m2);
	M3d_mat_mult_pt(m2, view, m2);
	...
	M3d_mat_mult_pt(m, k2, m);
	M3d_mat_mult_pt(m, view, m);
	
	M3d_mat_mult_pt(m1, k2, m1);
	M3d_mat_mult_pt(m1, view, m1);
	
	M3d_mat_mult_pt(m2, k2, m2);
	M3d_mat_mult_pt(m2, view, m2);
