...
int main ()
{
  ...
  // Build an xy cylinder
  f = fopen("cylinder.xyz","r") ;
  if (f == NULL) { printf("Can't open file %s\n", "cylinder.xyz") ;  exit(0) ;  }
  inherent_rgb[onum][0] = 0.6 ;
  inherent_rgb[onum][1] = 0.6 ;
  inherent_rgb[onum][2] = 0.6 ;
  read_xyz_file_into_data_structure (f, onum) ;
  fclose(f) ;

  nl = 0 ;
  tlist[nl] = SX ; plist[nl] = 2.82 ; nl++ ;
  tlist[nl] = RZ ; plist[nl] = 135  ; nl++ ;
  tlist[nl] = TX ; plist[nl] = 2.00 ; nl++ ;
  tlist[nl] = TY ; plist[nl] = 2.00 ; nl++ ;
  M3d_make_movement_sequence_matrix (V,Vi,  nl,tlist,plist) ;  
  ...
  nl = 0 ;
  tlist[nl] = SX ; plist[nl] = 2.82 ; nl++ ;
  tlist[nl] = RY ; plist[nl] = -135  ; nl++ ;
  tlist[nl] = TX ; plist[nl] = 2.00 ; nl++ ;
  tlist[nl] = TZ ; plist[nl] = 2.00 ; nl++ ;
  //tlist[nl] = RY ; plist[nl] = -90  ; nl++ ;
  M3d_make_movement_sequence_matrix (V,Vi,  nl,tlist,plist) ;  
  ...
  double eye[3], coi[3], up[3], eyespace[4][4], eyespaceI[4][4] ;
  ...
  while (1) {
    ...
    // put your code here!!!!!!!!!!!!
    M3d_view(eyespace, eyespaceI, eye, coi, up);
    //M3d_mat_mult_pt(eye, eyespace, eye);
    //M3d_mat_mult_pt(up, eyespace, up);

    double Iden[4][4] ;
    M3d_mat_mult(Iden, eyespace,eyespaceI) ;
    M3d_print_mat(Iden) ;
    
    for (int i=0; i<numobjects; i++) {M3d_mat_mult_points(x[i], y[i], z[i], eyespace, x[i], y[i], z[i], numpoints[i]);}
    
    G_rgb(0,0,0); G_clear();
    draw_all_objects();

    for (int i=0; i<numobjects; i++) {M3d_mat_mult_points(x[i], y[i], z[i], eyespaceI, x[i], y[i], z[i], numpoints[i]);}
